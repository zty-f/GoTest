...
type BusPublisher interface {
	...
	PublishWithPool(pool Pool, topic string, args ...interface{})
}
...
func (bus *EventBus) Publish(topic string, args ...interface{}) {
	bus.PublishWithPool(&globalDefaultPool, topic, args...)
}

func (bus *EventBus) PublishWithPool(pool Pool, topic string, args ...interface{}) {
	if handlers, ok := bus.handlers[topic]; ok && 0 < len(handlers) {
		...
		for i, handler := range copyHandlers {
			...
			if !handler.async {
				...
				copyHandler := handler
				pool.Submit(func() {
					bus.doPublishAsync(copyHandler, topic, args...)
				})
			}
		}
	}
...
var globalDefaultPool defaultPool

type defaultPool struct {
}

func (d *defaultPool) Submit(fn func()) {
	go fn()
}
